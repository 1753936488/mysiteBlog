...
from user.models import OAuthRelationship
...
class RegForm(forms.Form):
    ...
    password = forms.CharField(label='密码', min_length=6, widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '请输入密码'}))
    password_again = forms.CharField(label='重复密码', min_length=6, widget=forms.PasswordInput(
...
class ChangeNicknameForm(forms.Form):
    nickname_new = forms.CharField(label='新的昵称', max_length=20, min_length=1, widget=forms.TextInput(
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='请再次输入新密码', widget=forms.PasswordInput(
    ...
    def clean(self):
        ...
        if new_password == new_password_again:
            ...
            if new_password != old_password or new_password == '':
                raise forms.ValidationError('两次输入的密码不一致')
        return self.cleaned_data

    def clean_old_password(self):  # 验证旧密码是否正确
        old_password = self.cleaned_data.get('old_password', '')
